...
function extraireTexte(msg) {
  ...
  // WhatsApp peut emballer le vrai message dans plusieurs wrappers.
  for (let i = 0; i < 4 && message; i += 1) {
    const texte =
...
async function demarrerBot() {
  ...
  sock.ev.on('messages.upsert', async ({ messages = [], type }) => {
    console.log(`📥 messages.upsert | type=${type} | messages=${messages.length}`);
    ...
    for (const msg of messages) {
      ...
      const debugText = extraireTexte(msg).trim();
      console.log(
        `📩 Message reçu | chat=${chatId} | fromMe=${Boolean(msg.key?.fromMe)} | text=${JSON.stringify(debugText)}`
      );

      // Les messages envoyés par le compte du bot lui-même ne doivent pas
      // relancer les commandes, sinon !auto pourrait créer une boucle.
      ...
      try {
        ...
        /* !quiz... ou simplement quiz... */
        const commandeQuiz = body.replace(/^!/, '');
        if (/^quiz(?:\d+)?(?:\s+.*)?$/i.test(commandeQuiz)) {
