...
//
// Tolérances : majuscules, accents, ponctuation, espaces, déterminants
// (le/la/les/l'/un/une/des/du/de/d'/the…), particules (van/von/da/di…),
// « c'est … » / « je pense que … » en début de réponse, chiffres romains
// (Louis XIV = Louis 14 · XIXe = 19e), saint/st, œ/oe, « et » / « & »,
// et une faute de frappe isolée sur un mot d'au moins 6 lettres.
//
...
// Pas de nom court non plus pour un titre/une expression contenant « et », « au »,
// « des »… (« Guerre et Paix » n'accepte pas « Paix »).
...
// Longueur minimale d'un mot pour tolérer UNE faute de frappe (2 fautes à partir de +4).
// Mets 99 pour désactiver totalement la tolérance aux fautes.
const ANSWER_TYPO_MIN_LENGTH = 6;
...
// Distance d'édition (Levenshtein + transposition de deux lettres voisines).
...
function rawAnswerTokens(text) {
  const prepared = String(text ?? "")
    ...
    .replace(/(?<![\p{L}\p{N}])([MDCLXVI]{1,7})(?:er|ère|ere|ème|eme|e)(?![\p{L}\p{N}])/gu, (m, r) => {
...
function finishAnswerTokens(tokens) {
  ...
  const kept = toks.filter(t => !ANSWER_FUNCTION_WORDS.has(t));
...
function answerTokens(text) {
  return finishAnswerTokens(rawAnswerTokens(text));
}
...
function splitAnswerVariants(raw, depth = 0) {
  ...
  for (const piece of pieces) {
    ...
    for (const p of subs) {
      ...
      out.push(base || p.replace(/[\(\)\[\]]/g, " ").replace(/\s+/g, " ").trim());
  ...
  // On garde aussi la réponse entière (ex. « Pile ou face » tapée en entier).
  if (pieces.length > 1) {
    const whole = stripParens(text);
...
function answerTokenForms(spec) {
  const forms = [];
  const seen = new Set();
  for (const v of answerVariantsOf(spec)) {
...
// Index de toutes les réponses de la base : sert à détecter l'ambiguïté d'un nom court
// et à ne pas « corriger » une faute de frappe en un autre vrai mot de la base.
...
function getAnswerIndex() {
  ...
  const vocab = new Set();  // tous les mots présents dans les réponses
  ...
  for (const q of allQuestions()) {
    ...
    for (const v of answerVariantsOf(q)) countCase(v, false);
  ...
  answerIndexCache = { ngrams, vocab, common };
  return answerIndexCache;
}

// Faute de frappe isolée : mot attendu d'au moins 6 lettres, même première lettre,
// jamais sur un nombre, jamais si le mot tapé est lui-même un vrai mot de la base.
function fuzzyTokenMatch(p, c, index) {
  if (p === c) return true;
  if (/\d/.test(p) || /\d/.test(c)) return false;
  if (c.length < ANSWER_TYPO_MIN_LENGTH || p.length < ANSWER_TYPO_MIN_LENGTH - 1) return false;
  if (p[0] !== c[0]) return false;
  if (Math.abs(p.length - c.length) > 2) return false;
  if (index.vocab.has(p)) return false;
  const maxDist = c.length >= ANSWER_TYPO_MIN_LENGTH + 4 ? 2 : 1;
  return levenshtein(p, c) <= maxDist;
}

function answerTokensMatch(P, C, index) {
  if (!P.length || P.length !== C.length) return false;
  let fuzzyUsed = 0;
  for (let i = 0; i < P.length; i++) {
    if (P[i] === C[i]) continue;
    if (fuzzyUsed >= 1 || !fuzzyTokenMatch(P[i], C[i], index)) return false;
    fuzzyUsed++;
  }
  return true;
}

// « jeanjacques » = « jean jacques » · « jeanne darc » = « Jeanne d'Arc »
function answerCompactMatch(P, C) {
  if (P.compact && C.compact && P.compact.length >= 6 && P.compact === C.compact) return true;
  if (P.length === C.length) return false;
  const a = P.join("");
  return a.length >= 5 && a === C.join("");
}
...
function playerAnswerForms(raw) {
  ...
  if (!rawTokens.length || rawTokens.length > 14) return [];

  const forms = [];
  const base = finishAnswerTokens(rawTokens);
  base.compact = rawTokens.join("");
  ...
  if (k > 0 && k < rawTokens.length) {
    ...
    rest.compact = rawTokens.slice(k).join("");
...
function answerIsCorrect(answer, expected) {
  ...
  if (!rawAnswer.trim() || rawAnswer.length > 200) return false;
  ...
  for (const P of playerForms) {
    for (const C of expectedForms) {
      if (answerTokensMatch(P, C, index) || answerCompactMatch(P, C)) return true;
    }
  }

  // 2) Nom court : fin du nom uniquement, identifiante et non ambiguë.
  for (const C of expectedForms) {
    if (C.title || C.length < 2 || C.length > 5) continue;
    for (let i = 1; i < C.length; i++) {
      const S = C.slice(i);
      if (!isSafeShortForm(S, ownKey, index)) continue;
      for (const P of playerForms) {
        if (answerTokensMatch(P, S, index)) return true;
      }
    }
  }
...
async function askNext(chatId, sock) {
  ...
  const timeoutId = setTimeout(async () => {
    ...
    await send(sock, chatId, `Réponse : *${displayAnswer(q)}*`);
  ...
  if (s.auto && Math.random() * 100 < s.autoChance) {
    ...
    setTimeout(async () => {
      ...
      delete active[chatId];
      await send(sock, chatId, `*${displayAnswer(q)}*`);
...
async function handleMessage(msg, sock) {
  ...
  if (answerIsCorrect(raw, quiz.question)) {
    quiz.resolved = true;
    clearTimeout(quiz.timeoutId);
    active[chatId] = null;
    ...
    await send(sock, chatId, "✅");
    const board = scoreText(chatId, false);
    if (board) await send(sock, chatId, board);
    if (sessions[chatId]?.running) setTimeout(() => askNext(chatId, sock), NEXT_DELAY_MS);
  }
}
